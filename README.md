...
- 저장/불러오기
  ...
  - saveVersion 18이 아니거나 깨진 저장 데이터는 초기 상태로 복구
...
- 저장 구조는 `saveVersion: 18`이며 `spellId`, `itemId`, `reactionType`, `pendingReaction`, `pendingChoice`, `magicTicket`, `choice`, `multiItem`을 검증합니다.
...
- 보유 스킬 구조가 `GameState.skills`에 추가되었고 현재 저장 구조는 `saveVersion: 18`입니다.
...
- 저장 키와 저장 구조는 `saveVersion 18`입니다.

## 이번 단계 반영 사항 (saveVersion 18)

- 저장 키와 저장 구조는 `saveVersion 18`입니다. `angelTrial.claimedScores`, `angelTrial.lastScore`, `angelTrial.lastResult`를 저장/검증합니다.
...
- 저장 키는 `manrpg-pwa-ai:save:v18`, 저장 구조는 `saveVersion: 18`입니다.
...
- 저장 키와 저장 버전은 `manrpg-pwa-ai:save:v18`, `saveVersion: 18`입니다. v17 이하 저장 키는 legacy key로 정리 대상에 포함합니다.


## saveVersion 18 헤일로 9종 선택형 천사의 고리
...
- 증폭은 한 가지 행동의 AI 묘사를 무한히 증폭시킵니다. 실제 피해, 회복, 이동, 보상, 판정 결과는 바꾸지 않으며, AI 자동 GM 서술이 켜져 있을 때 다음 주요 행동의 묘사 프롬프트에 무한 증폭 지시를 추가합니다.
...
- saveVersion은 18입니다. 플레이어 1명, 적 1명, 보스 없음, 11x11 고정 맵, 랜덤 맵 없음, AI/Worker 비판정 원칙은 유지됩니다.

## saveVersion 19 대화형 TRPG GM 구조

- 프로젝트 목표는 GPTs 같은 대화형 TRPG 경험입니다. 플레이어는 항상 1명이고, 적은 화면에 상세 표시하지 않지만 코드 내부에서는 항상 1명만 유지합니다.
- 코드는 플레이어 HP/MP, 장비, 인벤토리, 스탯, 스킬, 마법, 코인, 숨은 적 내부 HP 같은 저장 상태 기억·수치 연산·검증·저장/불러오기를 담당합니다.
- API는 GM처럼 플레이어 자연어 행동 처리, 적 행동 선택/제안, 장면 전개, TRPG식 묘사를 담당합니다. 단, API는 전체 저장 상태를 소유하지 않으며 응답의 `stateDeltas`만 코드가 clamp하여 반영합니다.
- `gm-turn`, `enemy-action`, `compact-summary` LLM task를 추가했습니다. 기본 진행은 `gm-turn` 중심이며 응답은 JSON 구조로 제한됩니다.
- 토큰 절약을 위해 API에는 전체 `GameState`, 전체 시트, 전체 적 상세, 전체 로그를 보내지 않습니다. `buildGmTurnPayload`는 `sceneSummary` 300자 이하, `recentEvents` 최대 5개, 장비명만, 사용 가능 핵심 아이템명 최대 5개, 플레이어 핵심 수치, 숨은 적 힌트만 전달합니다.
- 보스몹 없음, 랜덤맵 없음, 11x11 고정 맵 유지를 계속 따릅니다.
- `ManRPG_v18_FINAL_병합패키지.zip` / `ManRPG_v18_FINAL_통합본.txt` 원본에 없는 스탯/아이템/장비/마법 효과는 창작하지 않습니다.
- 저장 키는 `manrpg-pwa-ai:save:v19`, 저장 구조는 `saveVersion: 19`입니다. v18 키는 legacy key 목록에 유지합니다.
