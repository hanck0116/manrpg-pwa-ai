...
현재 단계의 목표는 “층 클리어 후 원본 순서에 맞게 회복/정비, 5Lv 플러스, 보상 선택, 레벨업 스탯 분배, 다음 층 진입 흐름을 구현한다”입니다.
...
- 저장/불러오기 stub
  - `saveVersion: 5`
  - `floor`, `rewardState`, `inventory`, `levelUpPending`을 포함한 현재 구조만 유효 저장으로 취급
  - saveVersion 4 이하 데이터 또는 깨진 저장 데이터는 초기 상태로 복구
  - actionQueue 저장/복구
- 접이식 AI 설정 패널 자리
