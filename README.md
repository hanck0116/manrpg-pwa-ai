...
현재 단계의 목표는 “적 처치 후 층 클리어, 회복/정비, 보상 후보 생성, 보상 선택, 다음 층 진입 흐름을 구현한다”입니다.
...
- 층 클리어 회복/정비 후 `reward-pending`에서 외모 기반 보상 후보를 생성하고 선택
- 보상 확정 시 코인은 즉시 반영하고, 코인이 아닌 보상은 `inventory`에 보관
- 보상 확정 후 같은 7x7 고정 맵에 다음 층의 적 1명만 재생성
...
- 층 클리어/전투 종료 처리
  ...
  - `floor-cleared`와 `reward-pending`에서는 행동 추가/턴 마무리 불가
...
- 보상 선택 흐름
  ...
  - 보상 아이템 사용 효과는 아직 적용하지 않음
  - 보상 확정 후 다음 층 진입 버튼 활성화
- 인벤토리 요약
  - 접이식 패널에서 보유 아이템 이름과 개수를 표시
  - 코인은 플레이어 상태창에 계속 표시
- 다음 층/새 전투 시작
  - 보상 확정 후 다음 층 진입 가능
  - 다음 층 진입 시 `floor + 1`, `setupMode=false` 유지
...
- 저장/불러오기 stub
  - `saveVersion: 4`
  - `floor`, `rewardState`, `inventory`를 포함한 현재 구조만 유효 저장으로 취급
  - saveVersion 3 이하 데이터 또는 깨진 저장 데이터는 초기 상태로 복구
  - actionQueue 저장/복구
- 접이식 AI 설정 패널 자리
...
- 보상
  ...
  - 보상 효과는 아직 자동 적용하지 않고, 코인은 즉시 반영하며 코인 외 보상은 인벤토리에 저장
